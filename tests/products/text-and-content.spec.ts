import { standardUser, visualUser } from "@fixtures";
...
standardUser.describe(
  ...
  () => {
    ...
    standardUser(
      "should not contain suspicious patterns in product names or descriptions --> BUG: found in some entries @regression",
      async ({ productsPage }) => {
    ...
    standardUser(
      "should align product names correctly @regression",
      async ({ productsPage }) => {
    ...
    standardUser(
      "should not apply forbidden classes to Add to Cart buttons @regression",
      async ({ productsPage }) => {
...
visualUser.describe(
  "Products - content and style checks - visual user ",
  () => {
    ...
    visualUser(
      "should align product names correctly -->  BUG: some names are right-aligned @regression",
      async ({ productsPage }) => {
        const nameElements = await productsPage.getAllProductTitles.all();
        const misalignedNames: string[] = [];
    ...
    visualUser(
      "should not apply forbidden classes to Add to Cart buttons --> BUG: class btn_inventory_misaligned present @regression",
      async ({ productsPage }) => {
        const addToCartbuttons =
          await productsPage.getAddToCartButtonByRole.all();
        const misaligned: string[] = [];
...
standardUser.describe(
  ...
  () => {
    ...
    standardUser(
      "should compare product prices for standard, problem, and error users @regression",
